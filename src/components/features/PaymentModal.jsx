...
export function PaymentModal({
  ...
  if (isSuccess) {
    return <Modal isOpen={isOpen} onClose={() => { }} title="Payment Successful">
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title={`Join ${contest.name}`}>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Input label="Card Number" placeholder="0000 0000 0000 0000" {...register('cardNumber', {
        required: 'Card number is required',
        pattern: {
          value: /^\d{16}$/,
          message: 'Invalid card number'
        }
      })} error={errors.cardNumber?.message} icon={<CreditCard className="w-5 h-5 text-slate-500" />} />

      <div className="grid grid-cols-2 gap-4">
        <Input label="Expiry Date" placeholder="MM/YY" {...register('expiry', {
          required: 'Expiry is required'
        })} error={errors.expiry?.message} />
        <Input label="CVC" placeholder="123" type="password" {...register('cvc', {
          required: 'CVC is required',
          pattern: {
            value: /^\d{3,4}$/,
            message: 'Invalid CVC'
          }
        })} error={errors.cvc?.message} icon={<Lock className="w-4 h-4 text-slate-500" />} />
      </div>
